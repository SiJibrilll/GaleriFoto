<div>
    {{-- Knowing others is intelligence; knowing yourself is true wisdom. --}}    
        ...
        {{-- // TODO new discovery : foreach can be used for arrays now for some fucking reason. this will help us greatly --}}
       {{-- // TODO new discovery: this code below wont work because adding more posts will shift the division and makes the display twitcy again --}}
       {{-- But it almost worked in achiving our goal --}}
       {{-- we need to find a way to devide these posts and instead of refreshing the whole col array and creating a new one, we simply add to those col arrays --}}

       {{-- // TODO we could perhaps use the stored index way of doing things --}}
       <button wire:click='loadMore'>Load</button>
       {{count($columns[0])}}
        <div class="flex flex-row">
            @foreach ($columns as $index => $column)
            <div wire:key='{{$index}}' class="flex flex-col">
                @foreach ($column as $i => $post)
                    
                    {{-- @if ($i > floor(count($column) / 2))
                    <div x-intersect='$wire.loadMore'></div>                        
                    @endif --}}

                    {{-- post cards --}}
                    <div class="mb-2 max-w-44 overflow-hidden rounded-2xl main-{{$post[0]}}" onclick="window.location.href = '/posts/show/{{$post[0]}}';">
                        {{-- post thumbnail --}}
...
<script>
    ...
    function getScreenSize() {
        ...
        if (window.matchMedia('(min-width: 768px)').matches) {
            return 3 // if its larger than 768px, return tablet
        }
